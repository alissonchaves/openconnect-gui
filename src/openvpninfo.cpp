...
{
    const QString exeName = QStringLiteral("openvpn");
    ...
    const QStringList fallbackPaths = {
        QStringLiteral("/opt/homebrew/bin/openvpn"),
        QStringLiteral("/opt/homebrew/sbin/openvpn"),
        QStringLiteral("/usr/local/bin/openvpn"),
        QStringLiteral("/usr/local/sbin/openvpn"),
        QStringLiteral("/opt/local/bin/openvpn"),
        QStringLiteral("/opt/local/sbin/openvpn"),
        QStringLiteral("/usr/bin/openvpn"),
        QStringLiteral("/usr/sbin/openvpn"),
    };

    for (const QString& candidate : fallbackPaths) {
        QFileInfo fi(candidate);
        if (fi.exists() && fi.isFile() && fi.isExecutable()) {
            Logger::instance().addMessage(QObject::tr("OpenVPN found at %1").arg(candidate));
            return candidate;
        }
    }

    err = QObject::tr("OpenVPN binary not found (PATH and common locations)");
    return QString();
}
